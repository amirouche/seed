...
(define (test-roundtrip name input)
  (set! test-count (+ test-count 1))
  (let ([result (ast->src (parse input))])
...
(printf "~n=== Seed2 Step 1: Parse ===~n~n")
...
;; --- Roundtrip: parse → ast->src ---
(printf "── Roundtrip (parse → ast→src) ──~n")
...
(printf "~n=== Seed2 Step 2: Annotate ===~n~n")
...
(define (test-annotate-roundtrip name input)
  (set! test-count (+ test-count 1))
  (let ([result (l2->src ((annotate '()) (parse input)))])
    (if (equal? result input)
        (begin
          (set! test-pass (+ test-pass 1))
          (printf "  PASS ~a (roundtrip)~n" name))
...
;; x is local inside vau body because it's in scope from enclosing let
...
;; --- Roundtrip: parse → annotate → l2->src ---
(printf "── Roundtrip (parse → annotate → l2→src) ──~n")
(test-annotate-roundtrip "rt2/number" 42)
(test-annotate-roundtrip "rt2/var" 'x)
(test-annotate-roundtrip "rt2/quote" '(quote (a b)))
(test-annotate-roundtrip "rt2/if" '(if x y z))
(test-annotate-roundtrip "rt2/begin" '(begin 1 2 3))
(test-annotate-roundtrip "rt2/let" '(let ((x 5)) x))
(test-annotate-roundtrip "rt2/lambda" '(lambda (x) (+ x 1)))
(test-annotate-roundtrip "rt2/app" '(f 1 2))
(test-annotate-roundtrip "rt2/nested-lambda" '(lambda (x) (lambda (y) (+ x y))))
(test-annotate-roundtrip "rt2/letrec" '(letrec ((f (lambda (x) x))) (f 1)))
(test-annotate-roundtrip "rt2/vau" '(vau (x) e (eval x e)))
...
(printf "~n=== Seed2 Step 3: Classify ===~n~n")

;; Helper: parse → annotate → classify
...
(define (test-classify-roundtrip name input)
  (set! test-count (+ test-count 1))
  (let ([result (l3->src (pipeline input))])
    (if (equal? result input)
        (begin
          (set! test-pass (+ test-pass 1))
          (printf "  PASS ~a (roundtrip)~n" name))
...
(printf "── uses-eval? ──~n")
(test-pred "uses-eval?/const" uses-eval? '(const 42) #f)
(test-pred "uses-eval?/var" uses-eval? '(var x local) #f)
(test-pred "uses-eval?/call" uses-eval?
  '(call (var + free) ((var x local) (const 1))) #f)
(test-pred "uses-eval?/eval" uses-eval?
  '(eval (var x local) (var e local)) #t)
(test-pred "uses-eval?/nested-if" uses-eval?
  '(if (var x local) (eval (var y local) (var e local)) (const 1)) #t)
(test-pred "uses-eval?/deep-no" uses-eval?
  '(let ((x (const 5))) (call (var + free) ((var x local) (const 1)))) #f)

(printf "── has-free-vars? ──~n")
(test-pred "has-free-vars?/primitive" has-free-vars? '(var + free) #f)
(test-pred "has-free-vars?/non-prim" has-free-vars? '(var foo free) #t)
(test-pred "has-free-vars?/local" has-free-vars? '(var x local) #f)
(test-pred "has-free-vars?/const" has-free-vars? '(const 42) #f)
(test-pred "has-free-vars?/prim-call" has-free-vars?
  '(call (var + free) ((var x local) (const 1))) #f)
(test-pred "has-free-vars?/non-prim-call" has-free-vars?
  '(call (var foo free) ((var x local))) #t)

;; --- Classify tests ---
(printf "── Classify: fibonacci → lam ──~n")
(test-classify "fib→lam"
  '(letrec ((fib (lambda (n)
                   (if (<= n 1) n
                       (+ (fib (- n 1)) (fib (- n 2)))))))
     (fib 10))
...
(printf "── Classify: closure → lam ──~n")
(test-classify "closure→lam"
  '(let ((x 10)) (lambda (y) (+ x y)))
  '(let ((x (const 10)))
     (lam (y)
       (call (var + free) ((var x local) (var y local))))))

(printf "── Classify: non-prim free var → stays wrap(vau) ──~n")
(test-classify "free-var→wrap"
  '(lambda (x) (foo x))
  '(wrap (vau (x) #f
     (call (var foo free) ((var x local))))))

;; --- Roundtrip: parse → annotate → classify → l3->src ---
(printf "── Roundtrip (parse → annotate → classify → l3→src) ──~n")
(test-classify-roundtrip "rt3/number" 42)
(test-classify-roundtrip "rt3/var" 'x)
(test-classify-roundtrip "rt3/quote" '(quote (a b)))
(test-classify-roundtrip "rt3/if" '(if x y z))
(test-classify-roundtrip "rt3/begin" '(begin 1 2 3))
(test-classify-roundtrip "rt3/let" '(let ((x 5)) x))
(test-classify-roundtrip "rt3/lambda-pure" '(lambda (x) (+ x 1)))
(test-classify-roundtrip "rt3/app" '(f 1 2))
(test-classify-roundtrip "rt3/nested-lambda" '(lambda (x) (lambda (y) (+ x y))))
(test-classify-roundtrip "rt3/letrec" '(letrec ((f (lambda (x) x))) (f 1)))
(test-classify-roundtrip "rt3/vau" '(vau (x) e (eval x e)))
...
(printf "~n=== Seed2 Step 4: BTA ===~n~n")

;; Helper: full pipeline parse → annotate → classify → bta
(define (pipeline4 expr)
  (bta (classify ((annotate '()) (parse expr)))))
...
(define (test-bta-roundtrip name input)
  (set! test-count (+ test-count 1))
  (let ([result (l4->src (pipeline4 input))])
    (if (equal? result input)
        (begin
          (set! test-pass (+ test-pass 1))
          (printf "  PASS ~a (roundtrip)~n" name))
...
(define (test-bt name params ep body expected)
  (set! test-count (+ test-count 1))
  (let ([result (param-binding-times params ep body)])
    (if (equal? result expected)
        (begin
          (set! test-pass (+ test-pass 1))
          (printf "  PASS ~a~n" name))
...
;; --- param-binding-times unit tests ---
(printf "── param-binding-times ──~n")
...
(printf "── BTA: fibonacci → lam (no annotation) ──~n")
(test-bta "fib→lam-no-bta"
  '(letrec ((fib (lambda (n)
                   (if (<= n 1) n
                       (+ (fib (- n 1)) (fib (- n 2)))))))
     (fib 10))
...
(printf "── BTA: closure → lam (passes through) ──~n")
(test-bta "closure→lam"
  '(let ((x 10)) (lambda (y) (+ x y)))
  '(let ((x (const 10)))
     (lam (y)
       (call (var + free) ((var x local) (var y local))))))
...
;; --- Roundtrip: parse → annotate → classify → bta → l4->src ---
(printf "── Roundtrip (parse → annotate → classify → bta → l4→src) ──~n")
(test-bta-roundtrip "rt4/number" 42)
(test-bta-roundtrip "rt4/var" 'x)
(test-bta-roundtrip "rt4/quote" '(quote (a b)))
(test-bta-roundtrip "rt4/if" '(if x y z))
(test-bta-roundtrip "rt4/begin" '(begin 1 2 3))
(test-bta-roundtrip "rt4/let" '(let ((x 5)) x))
(test-bta-roundtrip "rt4/lambda-pure" '(lambda (x) (+ x 1)))
(test-bta-roundtrip "rt4/app" '(f 1 2))
(test-bta-roundtrip "rt4/nested-lambda" '(lambda (x) (lambda (y) (+ x y))))
(test-bta-roundtrip "rt4/letrec" '(letrec ((f (lambda (x) x))) (f 1)))
(test-bta-roundtrip "rt4/vau" '(vau (x) e (eval x e)))
...
(printf "~n=== Seed2 Step 5: Codegen + Driver ===~n~n")

;; --- Codegen unit tests ---
(printf "── Codegen: pure lam ──~n")

(define (test-codegen name input expected)
  (set! test-count (+ test-count 1))
...
(test-codegen "cg/const" '42 42)
(test-codegen "cg/string" '"hello" '(quote "hello"))
(test-codegen "cg/null" ''() ''())
(test-codegen "cg/quote" '(quote (a b)) '(quote (a b)))
(test-codegen "cg/if" '(if #t 1 2) '(if #t 1 2))
(test-codegen "cg/call" '(+ 1 2) '(+ 1 2))
(test-codegen "cg/lambda" '(lambda (x) (+ x 1)) '(lambda (x) (+ x 1)))
(test-codegen "cg/let" '(let ((x 5)) (+ x 1)) '(let ((x 5)) (+ x 1)))
...
;; Full vau codegen requires call-site dispatch (operatives control evaluation).
;; For now, test vau semantics via seed-eval interpreter.
(printf "── Execution: vau/eval (interpreter) ──~n")

(define (test-interp name input expected)
  (set! test-count (+ test-count 1))
  (let ([result (seed-eval input ground-env)])
    (if (equal? result expected)
        (begin
          (set! test-pass (+ test-pass 1))
          (printf "  PASS ~a~n" name))
...
;; Anaphoric-if: test result is passed to then-branch as argument
...
;; --- transform-to-letrec tests ---
(printf "── transform-to-letrec ──~n")

(set! test-count (+ test-count 1))
(let ([result (transform-to-letrec '((define (f x) (+ x 1)) (f 5)))])
...
(set! test-count (+ test-count 1))
(let ([result (transform-to-letrec '((define x 5) (define y 10) (+ x y)))])
...
(set! test-count (+ test-count 1))
(let ([result (transform-to-letrec '((+ 1 2)))])
  (if (equal? result '(+ 1 2))
...
(printf "~n=== Seed2 Step 6: Vau Call-Site Codegen ===~n~n")
...
;; --- FizzBuzz-like test (vau macros + lambda computation) ---
