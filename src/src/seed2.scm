...
;;     | (define <AST> <AST> <AST>)              ; define binding in target env
;;
...
(define (parse expr)
  (match expr
    ...
    ;; ---- Begin ----
    ;; If begin contains local (define ...) forms (2-arg: name+val or destructuring),
    ;; transform them into letrec first. 3-arg env-targeted defines are NOT local.
    [(begin . ,es)
     (if (exists (lambda (e)
                   (and (pair? e) (eq? (car e) 'define)
                        (or (= (length e) 3)                      ;; (define name val)
                            (and (= (length e) 3)                 ;; (define (names...) expr)
                                 (pair? (cadr e))))))
                 es)
         (parse (transform-to-letrec es))
         `(begin ,@(map parse es)))]
    ...
    [(let ,name ,bs ,body)
     (guard (symbol? name))
     (parse `(letrec ((,name (lambda ,(map car bs) ,body)))
               (,name ,@(map cadr bs))))]
    [(let ,name ,bs . ,bodies)
     (guard (symbol? name))
     (parse `(letrec ((,name (lambda ,(map car bs) ,@bodies)))
               (,name ,@(map cadr bs))))]
    ...
    ;; ---- define (3-arg: inject binding into target env) ----
    ;; (define env name expr) — all three sub-expressions are parsed
    [(define ,env-name ,name ,val)
     (guard (not (pair? name)))
     `(define ,(parse env-name) ,(parse name) ,(parse val))]

    ;; ---- define (4-arg: destructuring into target env) ----
    ;; (define env (names...) expr) — desugars to let + individual 3-arg defines
    [(define ,env-name (,names ...) ,expr)
     (let ([tmp (gensym "vals")])
       (parse `(let ((,tmp ,expr))
                 (begin ,@(let idx-loop ([ns names] [i 0] [acc '()])
                            (if (null? ns) (reverse acc)
                                (idx-loop (cdr ns) (+ i 1)
                                  (cons `(define ,env-name ,(car ns) (list-ref ,tmp ,i))
                                        acc))))))))]
...
(define (ast->src ast)
  (match ast
    ...
    [(vau ,p ,ep ,[body])
     `(vau ,p ,(or ep '_) ,body)]
    [(define ,[env-e] ,[name-e] ,[val-e]) `(define ,env-e ,name-e ,val-e)]
    [(eval ,[e] ,[ev]) `(eval ,e ,ev)]
    [(time ,[e]) `(time ,e)]
    [(call ,[op] (,[a] ...)) `(,op ,@a)]
    [,_ '???]))
...
(define (ast->src* ast)
  (match ast
    ...
    [(define ,env-e ,name-e ,val-e)
     `(define ,(ast->src* env-e) ,(ast->src* name-e) ,(ast->src* val-e))]
...
(define (subst-var-in-ast ast name replacement)
  (match ast
    ...
    [(define ,env-e ,name-e ,val-e)
     `(define ,(subst-var-in-ast env-e name replacement)
               ,(subst-var-in-ast name-e name replacement)
               ,(subst-var-in-ast val-e name replacement))]
    [,_ ast]))
...
(define (fuse-alist-lets ast)
  (match ast
    ...
    [(define ,env-e ,name-e ,val-e)
     `(define ,(fuse-alist-lets env-e) ,(fuse-alist-lets name-e) ,(fuse-alist-lets val-e))]
    [,_ ast]))
...
(define (spec ast subst ep ctx lenv ifuel)
  (match ast
    ...
    [(define ,env-e ,name-e ,val-e)
     `(define ,(spec env-e subst ep ctx lenv ifuel)
               ,(spec name-e subst ep ctx lenv ifuel)
               ,(spec val-e subst ep ctx lenv ifuel))]
...
;;   (quot x)       — vau param that received syntax 'x'
;;   (var x free)   — bare symbol reference in source (x used as name literal)
;;   (var x local)  — local variable (could be dynamic — not collectable)
(define (define-name-sym name-ast)
  (match name-ast
...
;; Only collects define! targeting caller env with a statically known symbol name.
;; Flattens nested begins and recurses into letrec bodies to find defines
;; surfaced by loop unrolling.
(define (collect-defines ast)
  (define (flatten-begins es)
    (cond
      [(null? es) '()]
      [(match (car es) [(begin ,inner ...) inner] [,_ #f])
       => (lambda (inner) (flatten-begins (append inner (cdr es))))]
      [else (cons (car es) (flatten-begins (cdr es)))]))
  (match ast
    [(begin ,es ...)
     (let loop ([es (flatten-begins es)] [names '()] [vals '()] [other '()])
       (if (null? es)
           (list (reverse names) (reverse vals)
                 (cond [(null? other) '(const #f)]
                       [(= (length other) 1) (car other)]
                       [else `(begin ,@(reverse other))]))
           (match (car es)
             [(define (dyn-env) ,name-e ,val-e)
              (let ([sym (define-name-sym name-e)])
                (if sym
                    (loop (cdr es) (cons sym names) (cons val-e vals) other)
                    (loop (cdr es) names vals (cons (car es) other))))]
             ;; Recurse into letrec bodies to find defines from unrolled loops
             [(letrec ,bs ,body)
              (let* ([inner (collect-defines body)]
                     [in (car inner)] [iv (cadr inner)] [ir (caddr inner)])
                (if (null? in)
                    ;; No defines found inside — treat as opaque
                    (loop (cdr es) names vals (cons (car es) other))
                    ;; Found defines — merge them, re-wrap remainder with letrec
                    (let ([rest (if (equal? ir '(const #f))
                                   '()
                                   (list `(letrec ,bs ,ir)))])
                      (loop (cdr es)
                            (append (reverse in) names)
                            (append (reverse iv) vals)
                            (append (reverse rest) other)))))]
             [,_ (loop (cdr es) names vals (cons (car es) other))])))]
    [(define (dyn-env) ,name-e ,val-e)
     (let ([sym (define-name-sym name-e)])
       (if sym
           (list (list sym) (list val-e) '(const #f))
           (list '() '() ast)))]
    [,_ (list '() '() ast)]))
...
;; Returns (wrappers core) where wrappers is a list of (let bindings)
;; or (letrec bindings) pairs, outermost first.
(define (unwrap-structural ast)
  (match ast
    [(let ,bs ,body)
     (let ([r (unwrap-structural body)])
       (list (cons `(let ,bs) (car r)) (cadr r)))]
    [(letrec ,bs ,body)
     (let ([r (unwrap-structural body)])
       (list (cons `(letrec ,bs) (car r)) (cadr r)))]
    [,_ (list '() ast)]))

;; Re-wrap compiled code with let/letrec layers (compiling the bindings).
(define (rewrap-codegen wrappers code ctx)
  (fold-right
    (lambda (w code)
      (match w
        [(let ,bs)
         `(let ,(map (lambda (b) (list (car b) (codegen* (cadr b) ctx))) bs)
            ,code)]
        [(letrec ,bs)
         `(letrec ,(map (lambda (b) (list (car b) (codegen* (cadr b) ctx))) bs)
            ,code)]))
    code
    wrappers))

;; codegen-begin: process a begin's expression list, restructuring
;; vau calls with define! into call-with-values chains.
;; Find a sub-expression vau call with defines inside an AST.
;; Returns #f or (list placeholder-sym vau-name vau-args ast-with-placeholder).
;; Only looks one level deep into call arguments.
(define (find-sub-vau-call ast ctx)
  (match ast
    [(call ,op ,args)
     (let scan ([as args] [i 0] [before '()])
       (if (null? as) #f
           (match (car as)
             [(call (var ,name local) ,vau-args)
              (guard (let ([entry (assq name ctx)])
                       (and entry (vau-info? (cdr entry))
                            (let ([info (cdr entry)])
                              (has-define? (cadddr info))))))
              (let ([ph (gensym "vau-result")])
                (list ph name vau-args
                      `(call ,op ,(append (reverse before)
                                          (cons `(var ,ph local) (cdr as))))))]
             [,_ (scan (cdr as) (+ i 1) (cons (car as) before))])))]
    [,_ #f]))

(define (codegen-begin exprs ctx)
  (let loop ([es exprs] [done '()])
    (cond
      [(null? es)
       ...
       (let ([e (car es)])
         (match e
           ...
           [(call (var ,name local) ,args)
            (guard (and (not (null? (cdr es)))  ;; must have continuation expressions
            ...
            (let* ([info (cdr (assq name ctx))]
                   ...
                   [specialized (specialize vau-body vau-params args vau-ep ctx)]
                   ;; Unwrap let/letrec layers to find the core with defines
                   [unwrapped (unwrap-structural specialized)]
                   [wrappers (car unwrapped)]
                   [core (cadr unwrapped)]
                   [defs (collect-defines core)]
                   [def-names (car defs)]
                   [def-vals (cadr defs)]
                   [stripped-body (caddr defs)])
              (if (null? def-names)
           ...
           [,_
            (let ([hoisted (and (not (null? (cdr es)))
                                (find-sub-vau-call e ctx))])
              (if hoisted
                  ;; Hoist: specialize the vau call, collect defines,
                  ;; wrap with call-with-values, compile remainder with new ctx
                  (let* ([ph (car hoisted)]
                         [vau-name (cadr hoisted)]
                         [vau-args (caddr hoisted)]
                         [modified-expr (cadddr hoisted)]
                         [info (cdr (assq vau-name ctx))]
                         [vau-params (cadr info)]
                         [vau-ep (caddr info)]
                         [vau-body (cadddr info)]
                         [specialized (specialize vau-body vau-params vau-args vau-ep ctx)]
                         [unwrapped (unwrap-structural specialized)]
                         [wrappers (car unwrapped)]
                         [core (cadr unwrapped)]
                         [defs (collect-defines core)]
                         ...
                         [stripped-body (caddr defs)])
                    (if (null? def-names)
                        ;; No defines found — compile normally
                        (loop (cdr es) (cons (codegen* e ctx) done))
                        ;; Hoist: wrap with call-with-values
                        (let* ([val-codes (map (lambda (v) (codegen* v ctx)) def-vals)]
                               ...
                               [producer `(lambda ()
                                            ,(rewrap-codegen wrappers inner-values ctx))]
                               [new-ctx (append (cons (cons ph 'scheme-var)
                                                      (map (lambda (n) (cons n 'scheme-var))
                                                           def-names))
                                                ctx)]
                               ;; Compile modified expr + continuation with new ctx
                               [cont (codegen-begin (cons modified-expr (cdr es)) new-ctx)]
                               [cwv `(call-with-values ,producer
                                       (lambda (news out)
                                         (call-with-values (lambda () (apply values news))
                                           (lambda ,def-names
  ...
  (match ast
    ...
    [(var ,n free)
     (cond
       ...
       ;; from the runtime env alist (they may be operative/keyword entries).
       [(and (not (memq n *primitives*))
             (assq '%has-env ctx))
        `(env-ref ',n env)]
       [else n])]
    [(quot ,d) `',d]
    ...
    [(letrec ((,names ,vals) ...) ,body)
     (let* ([binding-info
       ...
       (let* ([has-vau (exists (lambda (info) (eq? (cadr info) 'vau)) binding-info)]
              ;; Mark ctx so free vars in specialized vau bodies use env-ref
              [body-ctx (if has-vau (cons '(%has-env . #t) new-ctx) new-ctx)]
              [body-code (codegen* body body-ctx)])
         (if has-vau
    ...
    [(vau ,p ,ep ,body ,bta)
     (guard ep)
     (let* ([all-params (param-names p)]
            [has-def (has-define? body)]
            [new-ctx (cons '(%has-env . #t)
                       (append (if has-def '((%has-news . #t)) '())
                               (map (lambda (n) (cons n 'scheme-var)) all-params)
                               ctx))]
            [body* (codegen* body new-ctx)])
       `(cons 'operative
              (lambda (env . ,p)
                (let ([,ep env] ,@(if has-def '([%news '()]) '()))
                  ,(if has-def
                       `(let ([%result ,body*])
    ...
    [(vau ,p ,ep ,body)
     ...
     (let* ([all-params (param-names p)]
            [has-def (has-define? body)]
            [new-ctx (append (if has-def '((%has-news . #t)) '())
                             (map (lambda (n) (cons n 'scheme-var)) all-params)
                             ctx)]
            [body* (codegen* body new-ctx)])
       `(cons 'operative
    ...
    [(wrap (vau ,p ,ep ,body ,bta))
     ...
     (let* ([all-params (param-names p)]
            [new-ctx (cons '(%has-env . #t)
                       (append (map (lambda (n) (cons n 'scheme-var)) all-params) ctx))]
            [body* (codegen* body new-ctx)])
       `(lambda ,p
    ...
    ;; define! — inside operative body: immutable env extension + accumulate into %news
    ;;           outside (sub-expression fallback): mutate env alist
    [(define ,env-e ,name-e ,val-e)
     (let ([env-code (codegen* env-e ctx)]
           [name-code (codegen* name-e ctx)]
           [val-code (codegen* val-e ctx)])
       (if (assq '%has-news ctx)
           ;; Inside operative body — immutable extension + accumulate
           `(let ([v ,val-code])
              (set! env (cons (cons ,name-code v) env))
              (set! %news (cons (cons ,name-code v) %news))
              v)
           ;; Sub-expression fallback — mutate env alist
           `(let ([target ,env-code] [name ,name-code] [v ,val-code])
...
;; Used by begin/let to thread env through sequential expressions.
(define (seed-eval-stmt expr env)
  (define (thread-body es env)
    (let loop ([es es] [env env] [result #f])
  ...
  (match expr
    ;; Begin — thread env through elements
    [(begin . ,es) (thread-body es env)]
    ;; Let — thread env through bindings then body
    [(let ,bindings . ,bodies)
    ...
    ;; Local define (simple) — extend env immutably
    [(define ,name ,val)
    ...
    [(define (,names ...) ,expr)
     (let ([vals (seed-eval expr env)])
       (let loop ([ns names] [i 0] [env env])
         (if (null? ns) (values (void) env)
             (loop (cdr ns) (+ i 1)
                   (cons (cons (car ns) (list-ref vals i)) env)))))]
    ;; Application — operative calls propagate news into env
    [(,op . ,args)
     (guard (and (pair? expr)
                 (not (memq op '(quote if eval let letrec
                                  lambda vau define)))))
     (let ([proc (seed-eval op env)])
       (if (and (pair? proc) (eq? (car proc) 'operative))
           (call-with-values
             (lambda () (apply (cdr proc) env args))
             (lambda (news result)
               (values result (append news env))))
           (let ([vals (map (lambda (a) (seed-eval a env)) args)])
             (values (apply proc vals) env))))]
    ;; Everything else — env unchanged
    [,_ (values (seed-eval expr env) env)]))
...
(define (seed-eval expr env)
  (match expr
    ...
    [(let ,bindings . ,bodies)
     (let loop ([bs bindings] [env env])
       (if (null? bs)
           ;; Thread env through body expressions (like begin)
           (let body-loop ([es bodies] [env env] [result (void)])
             (if (null? es) result
                 (call-with-values
                   (lambda () (seed-eval-stmt (car es) env))
                   (lambda (val new-env)
                     (body-loop (cdr es) new-env val)))))
           (let* ([b (car bs)]
                  [name (car b)]
                  [val (seed-eval (cadr b) env)])
             (loop (cdr bs) (cons (cons name val) env)))))]
               ...
               (lambda (dyn-env . syntaxes)
                 (let* ([new-env (bind-params params syntaxes env)]
                        [new-env (if (or (eq? ep '_) (eq? ep '%ignore) (not ep))
                                     ...
                                     (cons (cons ep dyn-env) new-env))])
                   (values '() (seed-eval body new-env))))))]
    ;; define (destructuring, local) — (define (names...) expr)
    [(define (,names ...) ,expr)
     (let ([vals (seed-eval expr env)])
       (let loop ([ns names] [i 0])
         (if (null? ns) (void)
             (begin
               ...
               (set-car! env (cons (car ns) (list-ref vals i)))
               (loop (cdr ns) (+ i 1))))))]
    ;; define (simple, local) — (define name expr)
    [(define ,name ,val)
     (guard (symbol? name))
     (let ([v (seed-eval val env)])
       (set-cdr! env (cons (car env) (cdr env)))
       ...
       v)]
    ;; define (3-arg, target env) — (define env name expr)
    [(define ,target-env-expr ,name-expr ,val-expr)
     ...
     (let ([target-env (seed-eval target-env-expr env)]
       ...
       v)]
    ;; define (4-arg, destructuring into target env) — (define env (names...) expr)
    [(define ,target-env-expr (,names ...) ,expr)
     (let ([target-env (seed-eval target-env-expr env)]
           [vals (seed-eval expr env)])
       (let loop ([ns names] [i 0])
         (if (null? ns) (void)
             (begin
               (set-cdr! target-env (cons (car target-env) (cdr target-env)))
               (set-car! target-env (cons (car ns) (list-ref vals i)))
               (loop (cdr ns) (+ i 1))))))]
    ;; Application
    [(,op . ,args)
     (let ([proc (seed-eval op env)])
       (if (and (pair? proc) (eq? (car proc) 'operative))
           ;; Operative — pass env and unevaluated syntax, unpack (values news result)
           (call-with-values
             (lambda () (apply (cdr proc) env args))
             (lambda (news result) result))
...
(define ground-env
  `((+ . ,+) (- . ,-) (* . ,*) (/ . ,/)
    ...
    (list* . ,list*)
    (for-each . ,for-each)
    (iota . ,(lambda (n) (let loop ([i 0] [acc '()]) (if (= i n) (reverse acc) (loop (+ i 1) (cons i acc))))))
    ;; Disjoint encapsulation types (Kernel primitive)
    (make-encapsulation-type . ,(lambda ()
      (let ([tag (gensym "encap")])
        (list
          (lambda (val) (cons tag val))
          (lambda (x) (and (pair? x) (eq? (car x) tag)))
          (lambda (x) (cdr x))))))
    ;; Symbols for constructing expressions (used by provide, define-record-type etc.)
    ;; These are looked up via env-ref in vau bodies and placed into S-expressions for eval.
    ;; They must map to symbols (not operative objects) so seed-eval recognizes them.
...
;; Subsequent defines after a non-define create a new nested letrec.
(define (transform-to-letrec forms)
  ...
  ;; Process forms: collect defines into current bindings group,
  ;; emit non-define forms as body, recurse for remaining forms.
  (let loop ([fs forms] [bindings '()])
    (cond
      [(null? fs)
       ;; All forms were defines — body is void
       (wrap-letrec bindings '(void))]
      [else
       (let ([f (car fs)])
         (match f
           ;; (define (names...) expr) — destructuring bind
           ;; Emit current bindings as letrec, then nested let for destructuring,
           ;; then continue processing remaining forms as the body.
           [(define (,names ...) ,expr)
            (let* ([tmp (gensym "vals")]
                   [rest (loop (cdr fs) '())]
                   [inner `(let ((,tmp ,expr))
                             (let ,(let idx-loop ([ns names] [i 0] [acc '()])
                                     (if (null? ns) (reverse acc)
                                         (idx-loop (cdr ns) (+ i 1)
                                           (cons (list (car ns) `(list-ref ,tmp ,i)) acc))))
                               ,rest))])
              (wrap-letrec bindings inner))]
           ;; (define name val) — simple binding
           [(define ,name ,val)
            (guard (symbol? name))
            (loop (cdr fs) (cons (list name val) bindings))]
           ;; Non-define expression — emit current letrec, then continue
           [,expr
            (let ([rest (loop (cdr fs) '())])
              ;; rest is the result of processing remaining forms
              ;; Wrap: current bindings + (begin expr rest)
