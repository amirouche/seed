...
;;;
;;; Step 1: Parse (source → L1 AST)
;;;
;;; Key design: vau is the sole abstraction primitive.
;;; lambda desugars to (wrap (vau params #f body))
;;; where #f as env-param means "ignored" (canonical sentinel).
;;;
;;; Calling convention:
;;;   (vau (a b c . rest) env body ...) → (lambda (env a b c . rest) body-code)
;;;   (lambda (a b c . rest) body ...)  → (wrap (vau (a b c . rest) #f body ...))
;;;
...
(define (classify ast)
  (match ast
    ;; THE KEY RULE: wrap(vau with #f ep) → lam if pure
    [(wrap (vau ,p ,ep ,body))
     (guard (not ep))
...
;; Run a source expression through compile + eval
(define (run expr)
...
(define (run-file filename)
  ...
  (let* ([t0 (current-time 'time-monotonic)]
         [expr (transform-to-letrec (read-all-forms filename))]
         [code (compile expr)]
         [t1 (current-time 'time-monotonic)]
         [wrapped `(let ([env ',ground-env]) ,code)]
         [result (eval wrapped)]
         [t2 (current-time 'time-monotonic)]
         [ct (elapsed-seconds t0 t1)]
         [et (elapsed-seconds t1 t2)]
         [tt (+ ct et)]
         [cp (if (zero? tt) 0.0 (* 100.0 (/ ct tt)))]
         [ep (if (zero? tt) 0.0 (* 100.0 (/ et tt)))])
    (fprintf (current-error-port)
             "Timing: compile=~,3fs (~,1f%), execute=~,3fs (~,1f%), total=~,3fs~n"
             ct cp et ep tt)
    result))
...
(let ([args (command-line)])
  (cond
    [(and (> (length args) 2) (string=? (cadr args) "--dump"))
     ...
     (let loop ()
       ...
       (let ([expr (read)])
         (unless (eof-object? expr)
           (guard (exn [#t (display-condition exn) (newline)])
             (let ([result (run-with-env (transform-to-letrec (list expr)))])
